...
export const permissionsDialogReducer = createReducer(
  ...
  on(PermissionsDialogActions.remove, (state, {campusId}): State => {
    return {
      ...state,
      permissions: state.permissions.filter((campusIdFromList) => campusIdFromList !== campusId)
    }
  }),
  on(PermissionsDialogActions.edit, (state, {newValue, changedCampusId}): State => {
    return {
      ...state,
      permissions: state.permissions.map((campusId) => campusId === changedCampusId ? newValue : campusId)
    }
  }),
  on(PermissionsDialogApiActions.receivedPermissionsSuccessfully, (state, {permissions}): State => {
    return {
      ...state,
      permissions
    }
