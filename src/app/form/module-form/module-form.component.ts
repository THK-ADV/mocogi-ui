...
export type LocalizedInput<A, B> = {
  input: FormInput<A, B>
  language?: Language
}
...
export class ModuleFormComponent<A, B> implements OnInit {
  ...
  asMultipleOptions = (i: FormInput<A, B>) =>
    asMultipleOptionsInput(i) ??
    throwError(
      `expected form input to be multiple options input, but was ${i.kind}`,
    )

  asReadOnly = (i: FormInput<A, B>) =>
    asReadOnlyInput(i) ??
    throwError(`expected form input to be read only input, but was ${i.kind}`)

  asBoolean = (i: FormInput<A, B>) => i as BooleanInput

  formControl = (attr: string) => this.formGroup.get(attr) as FormControl
