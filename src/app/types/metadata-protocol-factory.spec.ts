...
describe('metadata protocol factory', () => {
  ...
  it('should parse a string', () => {
    expect(parseString('k1', { k1: 'v1' })).toEqual('v1')
    try {
  ...
  it('should parse optional', () => {
    expect(parseOptional('b', { a: 1, b: 2 }, (v) => v)).toEqual(2)
    expect(parseOptional('b', { a: 1, b: null }, (v) => v)).toBeUndefined()
    expect(parseOptional('b', { a: 1, b: undefined }, (v) => v)).toBeUndefined()
    expect(parseOptional('b', { a: 1 }, (v) => v)).toBeUndefined()
  })

  it('should parse title', () => {
    expect(parseTitle({ title: 'Paradigmen der Programmierung' })).toEqual(
      'Paradigmen der Programmierung',
    )
  })

  it('should parse abbreviation', () => {
    expect(parseAbbreviation({ abbreviation: 'PP' })).toEqual('PP')
  })
  ...
  it('should parse ects', () => {
    expect(parseECTS({ ects: 5 })).toEqual(5)
    expect(parseECTS({ ects: '5' })).toEqual(5)
  })
  ...
  it('should parse duration', () => {
    expect(parseDuration({ duration: 1 })).toEqual(1)
  })
  ...
  it('should parse workload', () => {
    ...
    expect(parseWorkload(workload)).toEqual(result)
  })
  ...
  it('should parse participants', () => {
    expect(
      parseParticipants({ participants: [{ value: { min: 0, max: 10 } }] }),
    ).toEqual(<Participants>{ min: 0, max: 10 })
    expect(parseParticipants({ participants: [] })).toBeUndefined()
  })
  ...
  it('should parse module relation', () => {
    const parent: { value: ModuleRelation } = {
      value: {
        ...
        children: [
          ...
          'c45bc346-61ae-446c-a98d-55abaef5729f',
        ],
      },
    }
    ...
    expect(parseModuleRelation({ 'module-relation': [parent] })).toEqual(
      parent.value,
    )
    expect(parseModuleRelation({ 'module-relation': [child] })).toEqual(
      child.value,
    )
    expect(parseModuleRelation({ 'module-relation': [] })).toBeUndefined()
  })
  ...
  it('should parse module coordinator', () => {
    ...
    expect(
      parseModuleManagement({ moduleCoordinator: moduleCoordinator }),
    ).toEqual(['cko'])
  })
  ...
  it('should parse lecturer', () => {
    ...
    expect(parseLecturers({ lecturer: lecturer })).toEqual(['ado', 'cko'])
    expect(parseLecturers({ lecturer: [] })).toEqual([])
  })
  ...
  it('should parse assessmentMethods', () => {
    ...
    const res1: AssessmentMethods = {
      ...
      optional: [
        { method: 'written-exam', percentage: undefined, precondition: [] },
      ],
    }
  ...
  it('should parse prerequisites', () => {
    ...
    const result: PrerequisitesOutput = {
      ...
      recommended: {
        ...
        modules: [
          'dca56fa6-1952-4b47-bf60-3dcb32e86ada',
          'f1fd21a9-ab5c-40b0-ab09-3dff35e86a72',
        ],
        pos: [],
      },
    }
  ...
  it('should parse competences', () => {
    ...
    expect(parseCompetences({ competences: competences })).toEqual([
      'analyze_domains',
    ])
  })
  ...
  it('should parse taught with', () => {
    ...
    expect(parseTaughtWith({ 'taught-with': taughtWith })).toEqual([
      'dca56fa6-1952-4b47-bf60-3dcb32e86ada',
    ])
  })
      ...
      'module-relation': [
        {
          value: {
            kind: 'parent',
            children: ['dca56fa6-1952-4b47-bf60-3dcb32e86ada'],
          },
        },
      ],
      moduleCoordinator: {
