...
function formatSemester(
  ...
  if (selectedStudyProgramId) {
    semester =
      module.studyProgram.find(
        (association) =>
          association.studyProgram.po.id === selectedStudyProgramId.poId,
      )?.recommendedSemester ?? [] // TODO adapt to specialization?
  } else {
...
const studyProgramAtomicOrd = Ordering.many<StudyProgramModuleAssociation>([
  ...
  Ordering.contraMap(
    stringOrd,
    ({ studyProgram }) => studyProgram.degree.deLabel,
  ),
])

function formatStudyPrograms(
  assoociations: StudyProgramModuleAssociation[],
): string[] {
  return assoociations.sort(studyProgramAtomicOrd).map((association) => {
